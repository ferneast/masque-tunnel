...
The client is correspondingly careful about its own reconnects. It caches the whole address set the last established tunnel resolved — not just the one that carried it — and dials that set if DNS is slow or failing, because with `--redirect-gateway` the tunnel's routes (and any `--dns` takeover, or the proxy-pushed resolver on iOS) intentionally stay installed while it is down, so the resolver may be unreachable exactly when it is needed. Keeping the full set matters when the path changes at the same time: a reconnect onto a Wi-Fi with no IPv6 can still reach the A record with the resolver down. A fresh answer, when one arrives, leads the order — `getaddrinfo` filters and ranks by the families the current path actually carries (RFC 6724), so it, not the cache, is the side that knows an address has stopped being reachable. For the same reason the client pins *every* candidate proxy address to the real gateway, not just the connected one, so a reconnect that lands on a different DNS answer or address family still escapes the split default.

Happy Eyeballs stays honest across all of this: a candidate wins the race only once its handshake completes, never on the connection object that 0-RTT resumption hands back before any packet has been acknowledged. Each attempt is also bounded at 10 seconds, so a candidate with no route on the current path — every send failing locally with `EHOSTUNREACH` — cannot hold up a reconnect that has no other candidate to fall back to.
