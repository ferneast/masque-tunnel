//! Server-side CONNECT-IP support (RFC 9484) on the quiche / tokio-quiche stack.
//!
...
//! sessions by destination address (each session's datagram flow), upstream
//! packets are source-validated and written to the TUN. Capsules travel on
//! the request stream body; IP packets travel on the datagram flow with an
//! RFC 9484 context ID of 0. Forwarding between the TUN and the internet is
//! the kernel's job (IP forwarding + NAT), which also handles the RFC 9484
//! §7.1 TTL decrement for transited packets.
...
/// Downstream target for one assigned client address: the session's datagram
/// flow sender and its flow ID.
#[derive(Clone)]
...
pub fn init(config: &IpConfig) -> Result<Arc<IpTunState>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Downstream router: TUN -> the datagram flow of the session that owns the
    // destination address.
    let st = state.clone();
    let mtu = config.mtu;
        ...
        loop {
            match st.tun.recv(&mut buf).await {
                Ok(n) => {
                    ...
                    let body = encode_context_payload(0, pkt);
                    let _ = flow
                        .sender
...
/// Handle a CONNECT-IP extended CONNECT request. The request has already been
/// authenticated and identified as `connect-ip` by the server. On success this
/// spawns the session's upstream and capsule tasks and returns; on failure it
/// has already sent an error status.
pub async fn handle_ip_request(
    state: &Arc<IpTunState>,
    stream_id: u64,
    path: &str,
    mut resp_send: OutboundFrameSender,
    resp_recv: InboundFrameStream,
    flow_send: OutboundFrameSender,
    flow_recv: InboundFrameStream,
) {
    let Some((target, ipproto)) = parse_connect_ip_path(path) else {
        log::info!("[server] Invalid CONNECT-IP path: {path}");
        reply_status(&mut resp_send, b"400").await;
        return;
    };
    ...
    if target != "*" || ipproto != "*" {
        ...
        reply_status(&mut resp_send, b"403").await;
        return;
    }
    ...
    if assigned.is_empty() {
        ...
        reply_status(&mut resp_send, b"503").await;
        return;
    }
    ...
    let _ = resp_send
        .send(OutboundFrame::Headers(
            vec![
                tokio_quiche::quiche::h3::Header::new(b":status", b"200"),
                tokio_quiche::quiche::h3::Header::new(b"capsule-protocol", b"?1"),
            ],
            None,
        ))
        .await;
    ...
    if resp_send.send(OutboundFrame::Body(assign, false)).await.is_err()
        || resp_send
            .send(OutboundFrame::Body(route_caps, false))
            .await
            .is_err()
    {
    ...
    log::info!(
        "[server] CONNECT-IP established: stream_id={stream_id} assigned={assigned_ips:?}"
    );
    ...
    // Session task: runs the upstream (flow -> TUN) and capsule loops until
    // either ends, then the guard drops and cleans up routes + pool.
    ...
    tokio::spawn(async move {
        let _guard = guard;
        tokio::select! {
            _ = upstream_loop(flow_recv, tun, assigned_ips.clone()) => {}
            _ = capsule_loop(resp_recv, resp_send, assigned_ips) => {}
        }
    });
}

/// Upstream: datagram flow -> TUN, with anti-spoofing source validation.
async fn upstream_loop(mut flow_recv: InboundFrameStream, tun: Arc<AsyncDevice>, assigned: Vec<IpAddr>) {
    while let Some(frame) = flow_recv.recv().await {
        let InboundFrame::Datagram(buf) = frame else {
            continue;
        };
        let Some((0, pkt)) = decode_context_payload(buf.as_slice()) else {
            continue;
        };
        match packet_src(pkt) {
            // try_send: never block; a full TUN queue drops, which IP tolerates.
            Some(src) if assigned.contains(&src) => {
                let _ = tun.try_send(pkt);
            }
            _ => log::trace!("[server] drop packet with invalid source (assigned {assigned:?})"),
        }
    }
}
...
async fn capsule_loop(
    mut resp_recv: InboundFrameStream,
    mut resp_send: OutboundFrameSender,
    assigned: Vec<IpAddr>,
) {
    let mut parser = CapsuleParser::default();
    while let Some(frame) = resp_recv.recv().await {
        let InboundFrame::Body(buf, _fin) = frame else {
            continue;
        };
        parser.push(&buf[..]);
        loop {
            match parser.next_capsule() {
                Ok(Some(capsule)) => handle_capsule(&mut resp_send, capsule, &assigned).await,
                Ok(None) => break,
                Err(e) => {
...
async fn handle_capsule(send: &mut OutboundFrameSender, capsule: Capsule, assigned: &[IpAddr]) {
    match capsule.capsule_type {
        CAPSULE_ADDRESS_REQUEST => {
            ...
            let _ = send
                .send(OutboundFrame::Body(encode_address_assign(&out), false))
                .await;
        }
